...
template<>
bool edit_component(Asset<Material> &component, const char *name, bool view_only)
{
  if (component)
    ImGui::Text("Material %s %s %s", name, component.asset_name().c_str(), component.is_copy() ? "(copy)" : "");
  else
    ImGui::Text("Material %s null", name);
  ...
  if (!view_only)
  {
    ...
    else
    {
      ...
      if (select)
      {
        ...
        const auto &resMap = Resources::instance().assets[nameOf<Material>::value];
        for (const auto &asset : resMap.resources)
        {
          names.push_back(asset.first.c_str());
        }
        static int curTex = -1;
        if (ImGui::ListBox("##Material", &curTex, names.data(), names.size(), 10))
        {
          select = false;
