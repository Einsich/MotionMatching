...
void AnimationPlayer::set_data_to_IK(const mat4 &t, int i, vec3 foot, vec3 toe, const char *foot_name, const char *toe_name)
{
...
void AnimationPlayer::update()
{
  ...
  if (transform)
  {
    ...
    set_data_to_IK(t, 0, leftFoot, leftToe, "LeftFoot", "LeftToeBase");
    set_data_to_IK(t, 1, rightFoot, rightToe, "RightFoot", "RightToeBase");
    
    vec3 toeNormal[2] = {leftToeNormal, rightToeNormal};
    ...
    for (int i = 0; i < 2; i++)
    {
      if (ikFoot[i].onGround)
      {
        ...
        process_IK(tree, cadr, t, footPosition, toeNormal[i], ikFoot[i].footNode, hipsNode);
        process_IK(tree, cadr, t, toePosition, toeNormal[i], ikFoot[i].toeNode, hipsNode);
      }
    }
